...
#define WGW_HAVE_MAC 2			// Have gateway router MAC 
...
{
#ifdef LOG_TO_UART					
		uartPutText("make_arp_answer_from_request\r\n", 30);
#endif	
        uint8_t i=0;
        //
        make_eth(buf);
...
#ifdef LOG_TO_UART	
        uartPutText("init_udp_or_www_server", 22);
#endif		
		uint8_t i=0;
        if (myip){
                while(i<4){
...
#ifdef WWW_server
uint16_t accept_www(uint8_t *buf, uint16_t plen)
	{
	uint16_t pos;
    ...
    if (buf[IP_PROTO_P]==IP_PROTO_TCP_V && buf[TCP_DST_PORT_H_P]==wwwport_h && buf[TCP_DST_PORT_L_P]==wwwport_l)
        ...
        if (buf[TCP_FLAGS_P] & TCP_FLAGS_SYN_V)
			...
			if (info_data_len > 0)		
				...
				make_tcp_ack_from_any(buf,0,0); //No data so close connection 
			}			
		}
	return 0;
	}						
...
uint16_t packetloop_arp_icmp_tcp(uint8_t *buf,uint16_t plen)
{
        ...
        if(plen==0)
			...
			if ((arpip_state & WGW_INITIAL_ARP || arpip_state & WGW_REFRESHING) && arp_delaycnt==0 && enc28j60linkup()) 
				{
#ifdef LOG_TO_UART					
				uartPutText("REPLY ARP\r\n", 11);
#endif					
				client_arp_whohas(buf,arpip);
				arpip_state |= WGW_ACCEPT_ARP_REPLY;
				}
			arp_delaycnt++;
			return 0;
			}
...
#define UART_NL				uartPutc('\r');	\
		...
		uint8_t ektcnt = enc28j60Read(EPKTCNT);
		uartPutTxt("EPKTCNT:");
		uartWriteUInt8(ektcnt);	
		UART_NL;		
        ...
        if(eth_type_is_arp_and_my_ip(buf,plen))
            ...
            if (buf[ETH_ARP_OPCODE_L_P]==ETH_ARP_OPCODE_REQ_L_V)
				{
#ifdef LOG_TO_UART					
				uartPutText("SERVICE ARP2\r\n", 13);
#endif								
                make_arp_answer_from_request(buf);
                }
			
				if (arpip_state & WGW_ACCEPT_ARP_REPLY && (buf[ETH_ARP_OPCODE_L_P]==ETH_ARP_OPCODE_REPLY_L_V) /*&& client_store_mac(gwip, gwmacaddr)*/)
				{
#ifdef LOG_TO_UART					
					uartPutText("WGW_HAVE_GW_MAC\r\n", 18);
#endif					
        ...
        if(buf[IP_PROTO_P]==IP_PROTO_ICMP_V && buf[ICMP_TYPE_P]==ICMP_TYPE_ECHOREQUEST_V)
            ...
            if (icmp_callback)
                ...
                // a ping packet, let's send ping
#ifdef LOG_TO_UART
            uartPutText("PING ECHO\r\n",11);
#endif			
		...
		if (plen<54 && buf[IP_PROTO_P]!=IP_PROTO_TCP_V )
			{
			//from here on we are only interested in TCP-packets; these are longer than 54 bytes
            // smaller than the smallest TCP packet and not tcp port
            return(0);
			}

#ifdef WWW_server
		return accept_www(buf, plen);
#endif // WWW_server
        return(0);
}
/* end of ip_arp_udp.c */
