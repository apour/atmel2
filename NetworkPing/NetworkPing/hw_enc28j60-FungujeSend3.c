...
void enc28j60PacketSend2(uint16_t len, uint8_t* packet) {
    uint8_t retry = 0;

	//uint8_t epkctn = enc28j60Read(EPKTCNT);
	//uartPutTxt("EPKTCNT:");
	//uartWriteUInt8(epkctn);
	//UART_NL;		
	//if (epkctn>0)
		//{
		//uartPutTxt("Receive in send\r\n");
		//uint16_t revCount = enc28j60PacketReceive2(200, &tempReceiveBuf[0]);
		//uartPutTxt("Receive count:");
		//uartWriteUInt16(revCount);
		//UART_NL;
		//}
...
//    resume_last_transmission:
		...
		//uartWriteUInt8(readPtr<<8);
		//uartWriteUInt8(readPtr&0xFF);
		UART_NL;		
    ...
    //#endif
		...
		//uint8_t eirReg = enc28j60Read(EIR);
        //uint16_t etxnd = readReg(ETXND);
		
#ifdef LOG_SENT_DATA
			uint8_t etxndl = enc28j60Read(ETXNDL);
			uint8_t etxndh = enc28j60Read(ETXNDH);
			uartPutc('p');
			uartPutc('o');
			uartPutc('s');
			uartPutc(':');
			...
			// read collision window
			etxndl = enc28j60Read(MACLCON2);
			uartPutc('C');
			uartPutc('L');
			uartPutc('C');
			uartPutc('O');
		...
		////readBuf(sizeof(transmit_status_vector), (byte*) &transmit_status_vector);
        //// LATECOL is bit number 29 in TSV (starting from 0)
//
        if (!((enc28j60Read(EIR) & EIR_TXERIF) && (transmit_status_vector[3] & 1<<5) /*tsv.transmitLateCollision*/) || retry > 16U) {
#ifdef LOG_SENT_DATA
			uartPutTxt("Break after cancel\r\n");
#endif			
...
void enc28j60PacketSend3(uint16_t len, uint8_t* packet)
	{
	// Set the write pointer to start of transmit buffer area
	writeReg(EWRPTL, TXSTART_INIT);
	// Set the TXND pointer to correspond to the packet size given
...
void enc28j60PacketSend(uint16_t len, uint8_t* packet)
	...
	uartPutc('\n');
#endif		
	enc28j60PacketSend3(len, packet);
	return;
	...
	/* AP - Test - Begin */
	uint8_t r = enc28j60ReadOp(ENC28J60_READ_CTRL_REG, ECON1);
	uartPutc('E');
	uartPutc('C');
	uartPutc('O');
	uartPutc('N');
	...
	uartPutc('S');
	uartPutc(':');
	uartWriteUInt8(eirReg);
	uartPutc('\r');
	uartPutc('\n');
#endif	
    //_delay_us(300);
	//if( (enc28j60Read(EIR) & EIR_TXERIF) ){
	if( (eirReg & EIR_TXERIF) )
		{
        enc28j60WriteOp(ENC28J60_BIT_FIELD_CLR, ECON1, ECON1_TXRTS);
        }
	}
